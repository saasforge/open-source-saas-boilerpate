...
from src.shared.utils.extensions import (
    db,
    db_schema,
    mail,
    alembic,
    ...
    for module in folder.iterdir():
        if module.is_dir():
            module_spec = importlib.util.find_spec(
                "src.{0}.{1}.{2}".format(folder_name, module.name, module_type)
            )
            if module_spec:
                component_module = importlib.import_module(
                    "src.{0}.{1}.{2}".format(
                        folder_name, module.name, module_type
                    )
                )
...
def register_components(app):
    """
    Automatically registers all module that need some initializing
    with application. To-do: make it not only for shared modules
    """
    ...
    __register_extensions_auto(
        services_modules_folder, app, "shared.services", "service"
    )
