...
models_folder += list(
    map(
        lambda path: Path.joinpath(Path.cwd(), path),
        glob.glob("src/modules/*/db_models"),
    )
)
...
for model_folder in models_folder:
    for module in model_folder.iterdir():
        if module.is_file():
            ...
            import_parts = module.parts[
                start_index:
            ]  # list(filter(lambda part: , module.parts))
...
def reinit_db(db_option=""):
    if db_option == "update":
        ...
        new_user = db_user_service.create_user(
            get_config_var("TEST_USER_NAME"),
            get_config_var("TEST_USER_EMAIL"),
            get_config_var("TEST_USER_PASS"),
            True,
        )
        new_user.save()
